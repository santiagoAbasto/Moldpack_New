...
function IntelligenceDashboard({data}){
  ...
  return <div className="intelligence-dashboard">
    <header className="intel-header"><div><span>SECURITY &amp; TRAFFIC INTELLIGENCE CENTER</span><h1>Vista general</h1><p>Tráfico, conversiones y señales operativas obtenidas del sitio real.</p></div><div className="intel-header-actions"><div className="intel-export-group"><a className="intel-export" href="/admin/database/export/sqlite"><Upload size={15}/>Respaldo local completo</a><a className="intel-export secondary" href="/admin/database/export/mysql"><Upload size={15}/>MySQL completo · phpMyAdmin</a></div><div className="intel-range" aria-label="Período">{['today','24h','7d','30d','90d'].map(range=><a key={range} className={data?.range===range?'active':''} href={`/dashboard?range=${range}`}>{range==='today'?'Hoy':range}</a>)}</div></div></header>
